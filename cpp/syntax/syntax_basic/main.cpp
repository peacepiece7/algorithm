...
void nullptrSummary();
void pointerVoldSummary();
void pointerVectorSummary();
void referenceSummary();
void functionBasicSummary(int, char*[]);
void recursionSummary();
void passByValueSummary();
void passByAddressSummary(int, char**);
...
void stringSummary() {
  ...
  std::cout << str5 << " " << std::to_string(true) << " " << str6 << " "
            << str6[5] << "\n";
...
void structSummary() {
  ...
  std::cout << person.height << " " << person.weight << " " << person.name
            << " " << person.grade << " " << person.eyeSight.left << " "
...
void unionSummary() {
  ...
  Product1 item1 = {1, 12};
  if (item1.idType == 0)
    std::cout << item1.idInteger
              << "\n";  // id의 type이 int인지 char 찾아보지 않으면 알 수 없음
  else
    std::cout << item1.idChars << "\n";

  std::cout << "union을 사용하면 아래와 같이 사용할 수 있음"
            << "\n";
  Product2 item2 = {
      2, {.idChars = "abc"}};  // 만들 때부터 int, char를 구분해서 만들어 둠
  Product2 item3 = {3, {.idInteger = 13}};
  if (item2.idType == 2)
    std::cout << item2.id.idChars << "\n";
  else
    std::cout << item3.id.idInteger << "\n";

  std::cout << "union안에서 align이 맞춰져서 padding(사용하지 않는 btye)이 더 "
               "작습니다\n";
...
void unscopedEnumSummary() {
  ...
  enum Color1 {
    ...
    Green,
    Alpha,
    SIZE  // 1, 2, 3, 4, 5의 값을 default로 가지는 거 같음
  };
  ...
  int textAttrs = 0;
  textAttrs |=
      TextAttribute::Bold;  // 범위지정 연산자, scope지정가능, c++11부터 지원
  textAttrs |= Underline;
...
void scopedEnumSummary() {
  ...
  enum class scopedColor { Blue, Red, Green, SIZE };
  ...
  int scopedColors[(int)scopedColor::SIZE] = {255, 128,
                                              64};  // String type Good!
  ...
  if (colors == (int)Colors::Red)
    printf("%s", "RED\n");
  else if (colors == (int)Colors::Green)
    printf("%s", "GREEN\n");
  else if (colors == (int)Colors::Blue)
    printf("%s", "BLUE\n");

  int colors2 = (int)(Colors)1;  // 0 == Red, 1 == Green, 2 == Blue
  if (colors2 == (int)Colors::Red)
    printf("%s", "RED\n");
  else if (colors2 == (int)Colors::Green)
    printf("%s", "GREEN\n");
  else if (colors2 == (int)Colors::Blue)
    printf("%s", "BLUE\n");
  ;
}
void array1Summary() {
  ...
  // copy(nums2, nums3 + 3 , nums2);  // integer pointer, +1당 4btye를 더함 =>
  // num3시작점 ~ num3시작점 + 12btye
  ...
  std::copy(&matrix[0][0], &matrix[0][0] + 3 * 3,
            &matrix1[0][0]);  // fill도 같음
...
void array2Summary() {
  ...
  std::cout << arr1.back() << arr1[arr1.size() - 1]
            << arr2[(sizeof(arr2) / sizeof(int)) - 1] << "\n";
  ...
  std::array<std::pair<int, int>, 5> a5 = {makePair(1, 2), makePair(3, 4),
                                           std::pair<int, int>(5, 6)};
}

std::pair<int, int> makePair(int a, int b) { return std::pair<int, int>(a, b); }
void array3Summary() {
  struct Person {
    float weight;
    float height;
  };
  Person persons[]{Person(70.7f, 175.6f), Person(77.7f, 180.5f)};

  std::cout << "1. rang-based for는 값을 가져온다."
            << "\n";
  for (Person person : persons) {  // for (Person& person : persons) => 이렇게
                                   // 주소를 가져오면 변함
    std::cout << person.weight << " ";
    person.weight =
        0;  // weight는 값이라서 persons[i].weight에 영향을 주지 못함
  };
  std::cout << "\n";
  ...
  std::cout << "\n";
  std::cout << "일반 for문으로 persons[i].weight를 건들면 참조 주소를 가져와서 "
               "값이 영구적으로 변함 : "
            << "\n";
...
void pointerBasicSummary() {
  ...
  */
  ...
  std::cout << "1.메모리의 주소, 포인터 => 변수의 주소를 값으로 가짐, 변수의 "
               "주소, 변수의 값"
            << "\n";
  int num = 147;
  int num2 = 147;
  ...
  *pointer = 0;  // *pointer == *&num
  ...
  Union u;  // union = 여러 속성 중 하나만 선택, 그럼 나머지 속성의 값은 어떻게
            // 변하나?
...
void pointerArraySummary() {
  ...
  // int pointer에 배열의 주소를 넣어서 사이즈가 달라짐 64bit라서 => 8btye,
  // nums=> 12btye
  ...
  std::cout << "(* __ptr64)[3], * __ptr64는 같음 typeid로 넘어가면서 배열이 "
               "붙어서 나오는 거 뿐"
            << "\n";
  ...
  /// asdasd0가 nums[0]일때, nums[0]의 주소값은는 4btye더해서 asdasd4가됨
  /// asdasd4는 nums[1]과 같음
  /// </summary>
...
void pointerStringSummary() {
  ...
  std::cout << "1. const char*"
            << "\n";
  // str1[0] =  'c'; // 이런짓이 불가능하게 char potiner는 const char*로
  // 선언해줘야함, char는 상수라서? 근데 int*도 아래처럼 변경 가능한데?
...
void dynamicAssignSummary() {
  ...
  Person* persons[2] = {new Person{56.5f, 177.3f}, new Person{77.7f, 150.55f}};
...
void nullptrSummary() {
  ...
  if (p != nullptr)
    ...
    cout << "p is nullptr"
         << "\n";
  cout << "2. func(nullptr)은 안됨 func(NULL)은 0이 감.. c11 이상시 "
          "nullptr사용하기"
       << "\n";
}
void pointerVoldSummary() {
  ...
  cout << "3. void*를 어따 씀 => 어떤 type의 포인터인지 알 수 없을 떄"
       << "\n";
  void* vp0 = malloc(sizeof(int) *
                     3);  // 12btye를  heap어딘가에 할당하고(동적 할당) 보이드
                          // 포인터로 반환함(메모리 주소를가르키는 포인터)
  int* intp = (int*)vp0;
...
void referenceSummary() {
  std::cout << "1. 참조 하기"
            << "\n";
  int num0 = 10;
  const int& num1 = num0;  // num0 값의 주소를 가진 변수, const -> (할당x,
                           // 변경x), const 뺴면 (할당o, 변경o)
  int* num2 = &num0;
  ...
  *num2 = num3;
  ...
  // 아래 코드를 비교해보면 기계어 코드가 같음 (내부적으로 reference는 pointer랑
  // 같음) reference는 포인터를 사용하기 쉽게 변형한 것 (오버로딩, 패스바이)
  /// <summary>
  /// int a = 3;
  /// int& b = a;
  /// int c = a + b;
...
void functionBasicSummary(int argc, char* argv[]) {
  ...
  // cd C:\Users\interbird\source\repos\peacepiece7\cpp-playground\x64\Debug에
  // .exe파일이 있음
...
void print(const Weapon* weapon) {
  ...
  // Weapon* weapon1 = weapon; const Weapon은 const로 받아야함 (포인터가
  // 가르키는 값을 변경을 못하도록)
}
void func(int nums[4]) {
  std::cout << typeid(nums).name()
            << "\n";  // pointer로 받기 때문에 배열의 사이즈 정보를
                      // 잃어버림(decay됨), 이래서 vector array권장함
...
void passByAddressSummary(int argc, char** argv) {
  ...
  std::cout << "3. 배열을 인자로 전달할 경우 배열 포인터로 받아서 "
               "속성(사이즈)를 잃어버림"
            << "\n";
  int nums[4]{1, 2, 3, 4};
  func(nums);
...
void passByReferenceSummary() {
  // 구조체처럼 사이즈가 클때, 원본을 변경할 수 없게 값을 전달하고 싶디면 const
  // int&를 사용 int같은 경우 간단하게 넘겨도 됨
  ...
  // const int&는 임시객체를 생성하고 임시 객체를 가르킴, 어차피 다른 값을 못
  // 가르키니까 10 같은 변수도 const int&에 할당 가능
  const int& n = 10;
}

int ppow(int base,
         int exp = 2) {  // int pow(int base = 2, int exp) 이렇게는 안됨
                         // default뒤 파라메터는 전부 default가 필요함
  int ret = 1;
  for (int i = 0; i < exp; i++) {
    ret *= base;
  }
...
// int ppow(int, int = 3); // 디폴트 매개변수를 한 번 더 쓰고 싶으면 각 파일에서
// 왼쪽 코드를 선언하면 됨
struct Person {
  float height;
  float weight;
...
int countDigit(std::string str) { return countDigit(stoi(str)); }
...
void functionOverloadingSummary() {
  ...
  cout << "3. cout 함수 오버로딩, cout함수 자세히 보기하면 다른 함수로 "
          "오버로딩됨"
       << "n";
  std::cout << 'a' << "\n";
  std::cout << (int)'a' << "\n";
...
void functionPointerSummary() {
  ...
  std::cout << "1. 함수 포인터 "
            << "\n";
  int (*f)(int, int) =
      &sum;  // int* f(int,int)는 int포인터를 반환하는거라 (*f)해야함
  ...
  // callbacks[(int)RequestType::Update]("John", "3333", "1234"); 이런건 어떻게
  // 할까?
